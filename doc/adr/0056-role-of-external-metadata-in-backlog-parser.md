...
- Where external metadata provides NCNs, these will take precedence over any NCNs derived from the document when populating `<uk:cite>` in the output
- The Backlog Parser will only directly construct and use derived data such as "Name" when generating a stub. When parsing a docx file it will defer to the normal parser logic which will continue to derive values when appropriate values cannot be directly obtained from the document text.
